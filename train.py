...
x = unet() 
x.compile(optimizer = keras.optimizers.Nadam(lr = 1e-3), loss ='binary_crossentropy', metrics = ['accuracy'])


tensorboard=keras.callbacks.TensorBoard(log_dir='log_dir', write_images=True)
#### for custom callback use :
# custom_callback = My_Callback() 
# change callback argument in fit_generator() argument

#### Uncomment to enable early stopping : ####
# earlystopper = EarlyStopping(patience=20, verbose=1)
checkpointer = ModelCheckpoint('ckpt_file', verbose=1)
results = x.fit_generator(generator=train_generator ,steps_per_epoch=40,validation_steps=5, validation_data=val_generator, epochs=100,callbacks=[ checkpointer,tensorboard])
'''
